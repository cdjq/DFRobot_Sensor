...
#define DFRobot_Sensor_I2C_ADDR 0x66 ///< 芯片IIC地址，无变化地址功能
...
public:
 ...
 /**
   ...
   * @n |    b7    |    b6    |    b5    |    b4    |    b3    |    b2   |    b1     |    b0    |
   * @n -----------------------------------------------------------------------------------------
   * @n |   ready  |         reversed               |      precision     | highspeed | lowpower |
   * @n -----------------------------------------------------------------------------------------
   * @attention 上电后，ready位默认为1，不可更改
   */
  typedef struct {
...
public:
   ...
   * @n       eLowPowerMode_t  低功耗模式
   * @n       eHighSpeedMode_t  高速模式
   * @n       ePrecisionMode_t  高精度模式
   * @return 返回uint8_t类型，用以表示切换模式的状态
   * @retval 0 操作成功
   * @retval other 操作失败
   */
  ...
  /**
   * @fn setLED(uint8_t r, uint8_t g, uint8_t b)
   * @brief 设置LED灯的颜色
   * @details  设置颜色后，0.2秒后生效
...
class DFRobot_Sensor_I2C:public DFRobot_Sensor{
public:
  DFRobot_Sensor_I2C(TwoWire *pWire=&Wire, uint8_t mode = eNomalPrecision|eNormalSpeed|eNormalPower);
